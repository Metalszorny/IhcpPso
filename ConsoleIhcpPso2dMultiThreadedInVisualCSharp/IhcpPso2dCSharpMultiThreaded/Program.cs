...
namespace IhcpPso2dCSharpMultiThreaded
{
    /// <summary>
    /// 
    /// </summary>
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < _particleSwarmSize; i++)
            {
                ...
                currentParticle.bestFitness1 = _particleBestFitness1[i];
                currentParticle.bestFitness2 = _particleBestFitness2[i];
                currentParticle.index = i;
                currentParticle.fitness1 = _particleFitness1[i];
                currentParticle.fitness2 = _particleFitness2[i];
                currentParticle.bestPosition1 = new double[_dimensionRowNumber];
                currentParticle.bestPosition2 = new double[_dimensionRowNumber];
                currentParticle.position1 = new double[_dimensionRowNumber];
                currentParticle.position2 = new double[_dimensionRowNumber];
                currentParticle.velocity1 = new double[_dimensionRowNumber];
                currentParticle.velocity2 = new double[_dimensionRowNumber];

                for (int j = 0; j < _dimensionRowNumber; j++)
                {
                    currentParticle.bestPosition1[j] = _particleBestPosition1[(i * _dimensionRowNumber) + j];
                    currentParticle.bestPosition2[j] = _particleBestPosition2[(i * _dimensionRowNumber) + j];
                    currentParticle.position1[j] = _particlePosition1[(i * _dimensionRowNumber) + j];
                    currentParticle.position2[j] = _particlePosition2[(i * _dimensionRowNumber) + j];
                    currentParticle.velocity1[j] = _particleVelocity1[(i * _dimensionRowNumber) + j];
                    currentParticle.velocity2[j] = _particleVelocity2[(i * _dimensionRowNumber) + j];
                }
            ...
            for (int i = 0; i < _particleSwarmSize; i++)
            {
                ...
                _particleFitness1[currentParticle.index] = currentParticle.fitness1;
                _particleFitness2[currentParticle.index] = currentParticle.fitness2;
                _particleBestFitness1[currentParticle.index] = currentParticle.bestFitness1;
                _particleBestFitness2[currentParticle.index] = currentParticle.bestFitness2;

                for (int j = 0; j < _dimensionRowNumber; j++)
                {
                    _particlePosition1[(currentParticle.index * _dimensionRowNumber) + j] = currentParticle.position1[j];
                    _particlePosition2[(currentParticle.index * _dimensionRowNumber) + j] = currentParticle.position2[j];
                    _particleVelocity1[(currentParticle.index * _dimensionRowNumber) + j] = currentParticle.velocity1[j];
                    _particleVelocity2[(currentParticle.index * _dimensionRowNumber) + j] = currentParticle.velocity2[j];
                    _particleBestPosition1[(currentParticle.index * _dimensionRowNumber) + j] = currentParticle.bestPosition1[j];
                    _particleBestPosition2[(currentParticle.index * _dimensionRowNumber) + j] = currentParticle.bestPosition2[j];
                }
            }
        ...
        /// <param name="g"></param>
        /// <returns></returns>
        private static double[,] CalculateCooldownCurve2D(Particle currentParticle, double[,] currentTemperature, double[,] previousTemperature, double[,] g)
        {
            double temperature = 0;
            double heatConductivity = 0;
            double alpha = 0;
            ...
            for (int i = 1; i < _horizontalSplitting - 1; i++)
            {
                ...
                currentTemperature[i, _verticalSplitting - 1] =
                    2 * _rY * alpha * (previousTemperature[i, _verticalSplitting - 2] -
                    previousTemperature[i, _verticalSplitting - 1] + (_dY / heatConductivity) *
                    GetHeatTransferCoefficient(temperature, currentParticle, i, _verticalSplitting - 1) *
                    (_enviromentTemperature - previousTemperature[i, _verticalSplitting - 1])) +
                    _rX * alpha * (previousTemperature[i - 1, _verticalSplitting - 1] -
                    2 * previousTemperature[i, _verticalSplitting - 1] +
                    previousTemperature[i + 1, _verticalSplitting - 1]) +
            ...
            for (int i = 1; i < _horizontalSplitting - 1; i++)
            {
                ...
                currentTemperature[i, 0] =
                    2 * _rY * alpha * (previousTemperature[i, 1] - previousTemperature[i, 0] + (_dY / heatConductivity) *
                    GetHeatTransferCoefficient(temperature, currentParticle, i, 0) *
                    (_enviromentTemperature - previousTemperature[i, 0])) + _rX * alpha *
                    (previousTemperature[i - 1, 0] - 2 * previousTemperature[i, 0] + previousTemperature[i + 1, 0]) +
                    (_timeDifference / _dX) * (1 / (2 * _radius)) * alpha *
                    Math.Abs(previousTemperature[i - 1, 0] - previousTemperature[i, 1]) +
            ...
            if (!_partlyCooling)
            {
                ...
                for (int i = 1; i < _verticalSplitting - 1; i++)
                {
                    ...
                    currentTemperature[_horizontalSplitting - 1, i] =
                        ...
                        (previousTemperature[_horizontalSplitting - 1 - 1, i] -
                        previousTemperature[_horizontalSplitting - 1, i] + (_dX / heatConductivity) *
                        GetHeatTransferCoefficient(temperature, currentParticle, _horizontalSplitting - 1, i) *
                        (_enviromentTemperature - previousTemperature[_horizontalSplitting - 1, i])) +
                        (_timeDifference / heatConductivity) * (1 / _radius) * alpha *
                        GetHeatTransferCoefficient(temperature, currentParticle, _horizontalSplitting - 1, i) *
                        (_enviromentTemperature - previousTemperature[_horizontalSplitting - 1, i]) +
            ...
            currentTemperature[0, 0] =
                previousTemperature[0, 0] + 4 * _rX * alpha * (previousTemperature[1, 0] -
                previousTemperature[0, 0]) + 2 * _rY * alpha * (previousTemperature[0, 1] - previousTemperature[0, 0] +
                (_dY / heatConductivity) * GetHeatTransferCoefficient(temperature, currentParticle, 0, 0) *
                (_enviromentTemperature - previousTemperature[0, 0])) + (_timeDifference / heatConductivity) * alpha;
            // Inner top corner.
            //temperature = previousTemperature[_verticalSplitting - 1];
            //heatConductivity = GetHeatConductivity(temperature);
            ...
            currentTemperature[0, _verticalSplitting - 1] =
                ...
                previousTemperature[0, _verticalSplitting - 1]) + 2 * _rY * alpha *
                (previousTemperature[0, _verticalSplitting - 2] - previousTemperature[0, _verticalSplitting - 1] + (_dY / heatConductivity) *
                GetHeatTransferCoefficient(temperature, currentParticle, 0, _verticalSplitting - 1) *
                (_enviromentTemperature - previousTemperature[0, _verticalSplitting - 1])) + (_timeDifference / heatConductivity) * alpha;
            // Outer bottom corner.
            //temperature = previousTemperature[((_horizontalSplitting - 1) * _verticalSplitting)];
            //heatConductivity = GetHeatConductivity(temperature);
            ...
            alpha = GetAlpha(heatConductivity, GetSpecificHeat(temperature));
            currentTemperature[_horizontalSplitting - 1, 0] =
                2 * _rY * alpha * ((_dY / heatConductivity) * GetHeatTransferCoefficient(temperature, currentParticle, _horizontalSplitting - 1, 0) *
                (_enviromentTemperature - previousTemperature[_horizontalSplitting - 1, 0]) -
                previousTemperature[_horizontalSplitting - 1, 0] + previousTemperature[_horizontalSplitting - 1, 1]) +
                2 * _rX * alpha * ((_dX / heatConductivity) * GetHeatTransferCoefficient(temperature, currentParticle, _horizontalSplitting - 1, 0) *
                (_enviromentTemperature - previousTemperature[_horizontalSplitting - 1, 0]) -
                previousTemperature[_horizontalSplitting - 1, 0] + previousTemperature[_horizontalSplitting - 2, 0]) +
                (_timeDifference / heatConductivity) * (1 / _radius) * alpha *
                GetHeatTransferCoefficient(temperature, currentParticle, _horizontalSplitting - 1, 0) *
                (_enviromentTemperature - previousTemperature[_horizontalSplitting - 1, 0]) +
                (_timeDifference / heatConductivity) * alpha + previousTemperature[_horizontalSplitting - 1, 0];
            // Outer top corner.
            //temperature = previousTemperature[((_horizontalSplitting - 1) * _verticalSplitting) + (_verticalSplitting - 1)];
            ...
            currentTemperature[_horizontalSplitting - 1, _verticalSplitting - 1] =
                2 * _rY * alpha * (previousTemperature[_horizontalSplitting - 1, _verticalSplitting - 2] -
                previousTemperature[_horizontalSplitting - 1, _verticalSplitting - 1] + (_dY / heatConductivity) *
                GetHeatTransferCoefficient(temperature, currentParticle, _horizontalSplitting - 1, _verticalSplitting - 1) *
                (_enviromentTemperature - previousTemperature[_horizontalSplitting - 1, _verticalSplitting - 1])) + 2 * _rX * alpha *
                (previousTemperature[_horizontalSplitting - 2, _verticalSplitting - 1] -
                previousTemperature[_horizontalSplitting - 1, _verticalSplitting - 1] + (_dX / heatConductivity) *
                GetHeatTransferCoefficient(temperature, currentParticle, _horizontalSplitting - 1, _verticalSplitting - 1) *
        ...
        /// <param name="y"></param>
        /// <returns></returns>
        private static double GetHeatTransferCoefficient(double temperature, Particle currentParticle, int x, int y)
        {
            ...
            else
            {
                if (_dimensionRowNumber > 0)
                {
                    ...
                    while ((i < (_dimensionRowNumber * _dimensionColumnNumber)) &&
                        (_htcValues[i + 0] <= temperature))
                    {
                        heatTransferCoefficient0[0] = currentParticle.position1[(i / _dimensionColumnNumber)];
                        heatTransferCoefficient0[1] = currentParticle.position2[(i / _dimensionColumnNumber)];
                        temperature0 = _htcValues[i + 0];
                        i += _dimensionColumnNumber;
                    }
                    ...
                    if (i < (_dimensionRowNumber * _dimensionColumnNumber))
                    {
                        heatTransferCoefficient1[0] = currentParticle.position1[(i / _dimensionColumnNumber)];
                        heatTransferCoefficient1[1] = currentParticle.position2[(i / _dimensionColumnNumber)];
                        temperature1 = _htcValues[i + 0];
                    ...
                    {
                        double htc0 = currentParticle.position1[(_dimensionRowNumber - 1)];
                        double htc1 = currentParticle.position2[(_dimensionRowNumber - 1)];
                        double result = (htc0 < htc1 ? htc0 : htc1) + (Math.Abs(htc0 - htc1) * ((y + 1.0) / _verticalSplitting));
        ...
        private static Particle InitializeParticle(Particle currentParticle)
        {
            ...
            double[] fitnessValues = ObjectiveFunction(currentParticle);
            currentParticle.fitness1 = fitnessValues[0];
            currentParticle.fitness2 = fitnessValues[1];
            currentParticle.bestFitness1 = currentParticle.fitness1;
            currentParticle.bestFitness2 = currentParticle.fitness2;

            for (int j = 0; j < _dimensionRowNumber; j++)
            {
                currentParticle.bestPosition1[j] = currentParticle.position1[j];
                currentParticle.bestPosition2[j] = currentParticle.position2[j];
            }
        ...
        public static double[] ObjectiveFunction(Particle currentParticle)
        {
            ...
            {
                currentTemperature = CalculateCooldownCurve2D(currentParticle, currentTemperature, previousTemperature, g);
        ...
        private static void Optimize()
        {
            ...
            while (_epoch < _maxEpochs && staticEpochs < _maxStaticEpochs)
            {
                ...
                for (int i = 0; i < _particleSwarmSize; i++)
                {
                    ...
                    Task<Particle> newTask = new Task<Particle>(() =>
                    {
                        return OptimizePosition(currentParticle, mBest1, mBest2, beta);
                    });
                    newTask.Start();
                    taskArray[i] = newTask;
                }
        ...
        private static Particle OptimizePosition(Particle currentParticle, double mBest1, double mBest2, double beta)
        {
            switch (_particleOptimalisationType)
            {
                case 1:
                    ...
                    break;
                case 2:
                    currentParticle = UpdateQuantumPosition(currentParticle, mBest1, mBest2, beta);
                    break;
            }
        ...
        {
            return UpdatePosition(UpdateVelocity(currentParticle, GetBestLocalPositions(currentParticle.index, 1), GetBestLocalPositions(currentParticle.index, 2)));
        }
        ...
        {
            if (CheckIsInRange(currentParticle.position1) && CheckIsInRange(currentParticle.position2))
            {
                double[] fitnessValues = ObjectiveFunction(currentParticle);
                currentParticle.fitness1 = fitnessValues[0];
                currentParticle.fitness2 = fitnessValues[1];

                if (currentParticle.fitness1 < currentParticle.bestFitness1)
                {
                    currentParticle.bestFitness1 = currentParticle.fitness1;

                    for (int i = 0; i < _dimensionRowNumber; i++)
                    {
                        currentParticle.bestPosition1[i] = currentParticle.position1[i];
                    }
                }

                if (currentParticle.fitness2 < currentParticle.bestFitness2)
                {
                    currentParticle.bestFitness2 = currentParticle.fitness2;

                    for (int i = 0; i < _dimensionRowNumber; i++)
                    {
                        currentParticle.bestPosition2[i] = currentParticle.position2[i];
                    }
                }
            }
        ...
        private static Particle UpdatePosition(Particle currentParticle)
        {
            for (int i = 0; i < _dimensionRowNumber; i++)
            {
                currentParticle.position1[i] = (currentParticle.position1[i] + currentParticle.velocity1[i]);
                currentParticle.position2[i] = (currentParticle.position2[i] + currentParticle.velocity2[i]);
            }
        ...
        /// <param name="beta"></param>
        private static Particle UpdateQuantumPosition(Particle currentParticle, double mBest1, double mBest2, double beta)
        {
            for (int i = 0; i < _dimensionRowNumber; i++)
            {
                double fi = StaticRandom.NextDouble();
                double p = fi * currentParticle.bestPosition1[i] + (1 - fi) * 
                    _globalBestPosition1[(_globalBestSize - 1 - _dimensionRowNumber) + i];

                if (fi > 0.5)
                {
                    currentParticle.position1[i] = p - beta *
                        Math.Abs(mBest1 - currentParticle.position1[i]) * (-Math.Log10(fi));
                }
                else
                {
                    currentParticle.position1[i] = p + beta *
                ...
                if (currentParticle.position1[i] < _rangeMin)
                {
                    currentParticle.position1[i] = 2 *
                        _rangeMin - currentParticle.position1[i];
                }

                if (currentParticle.position1[i] > _rangeMax)
                {
                    currentParticle.position1[i] = 2 *
                        _rangeMax - currentParticle.position1[i];
                }

                fi = StaticRandom.NextDouble();
                p = fi * currentParticle.bestPosition2[i] +
                    (1 - fi) * _globalBestPosition2[(_globalBestSize - 1 - _dimensionRowNumber) + i];

                if (fi > 0.5)
                {
                    currentParticle.position2[i] = p - beta *
                        Math.Abs(mBest2 - currentParticle.position2[i]) * (-Math.Log10(fi));
                }
                else
                {
                    currentParticle.position2[i] = p + beta *
                ...
                if (currentParticle.position2[i] < _rangeMin)
                {
                    currentParticle.position2[i] = 2 *
                        _rangeMin - currentParticle.position2[i];
                }

                if (currentParticle.position2[i] > _rangeMax)
                {
                    currentParticle.position2[i] = 2 *
                        _rangeMax - currentParticle.position2[i];
                }
            }
        ...
        private static Particle UpdateVelocity(Particle currentParticle, double[] bestLocalPosition1, double[] bestLocalPosition2)
        {
            for (int i = 0; i < _dimensionRowNumber; i++)
            {
                ...
                currentParticle.velocity1[i] =
                    (_weight * currentParticle.velocity1[i]) +
                    (_particleConstant1 * StaticRandom.NextDouble() *
                    (currentParticle.bestPosition1[i] -
                    currentParticle.position1[i])) +
                    (_particleConstant2 * StaticRandom.NextDouble() *
                    (bestLocalPosition1[i] - currentParticle.position1[i]));
                currentParticle.velocity2[i] =
                    (_weight * currentParticle.velocity2[i]) +
                    (_particleConstant1 * StaticRandom.NextDouble() *
                    (currentParticle.bestPosition2[i] -
                    currentParticle.position2[i])) +
                    (_particleConstant2 * StaticRandom.NextDouble() *
                    (bestLocalPosition2[i] - currentParticle.position2[i]));
            }
        ...
        public static Particle GetInitialPosition(Particle currentParticle)
        {
            for (int i = 0; i < _dimensionRowNumber; i++)
            {
                ...
                {
                    currentParticle.position1[i] = ((_rangeMax - _rangeMin) * StaticRandom.NextDouble() + _rangeMin);
                }
                // There is only one given value.
                else if (_particleInitialPositionSize == 1)
                {
                    currentParticle.position1[i] = _particleInitialPosition[0];
                }
                // There are as many given value as the position dimensions.
                else if (_particleInitialPositionSize == _dimensionRowNumber)
                {
                    currentParticle.position1[i] = _particleInitialPosition[i];
                }
            ...
            for (int i = 0; i < _dimensionRowNumber; i++)
            {
                ...
                if (_particleInitialPosition == null)
                {
                    currentParticle.position2[i] = ((_rangeMax - _rangeMin) * StaticRandom.NextDouble() + _rangeMin);
                }
                // There is only one given value.
                else if (_particleInitialPositionSize == 1)
                {
                    currentParticle.position2[i] = _particleInitialPosition[0];
                }
                // There are as many given value as the position dimensions.
                else if (_particleInitialPositionSize == _dimensionRowNumber)
                {
                    currentParticle.position2[i] = _particleInitialPosition[i];
                }
                // The current position can be set from the given values.
                else if (i < _particleInitialPositionSize)
                {
        ...
        public static Particle GetInitialVelocity(Particle currentParticle)
        {
            for (int i = 0; i < _dimensionRowNumber; i++)
            {
                ...
                {
                    currentParticle.velocity1[i] = ((_rangeMax - _rangeMin) * StaticRandom.NextDouble() + _rangeMin);
                }
                //  There is only one given value.
                else if (_particleInitialVelocitySize == 1)
                {
                    currentParticle.velocity1[i] = _particleInitialVelocity[0];
                }
                // There are as many given value as the velocity dimensions.
                else if (_particleInitialVelocitySize == _dimensionRowNumber)
                {
                    currentParticle.velocity1[i] = _particleInitialVelocity[i];
                }
            ...
            for (int i = 0; i < _dimensionRowNumber; i++)
            {
                ...
                if (_particleInitialVelocity == null)
                {
                    currentParticle.velocity2[i] = ((_rangeMax - _rangeMin) * StaticRandom.NextDouble() + _rangeMin);
                }
                //  There is only one given value.
                else if (_particleInitialVelocitySize == 1)
                {
                    currentParticle.velocity2[i] = _particleInitialVelocity[0];
                }
                // There are as many given value as the velocity dimensions.
                else if (_particleInitialVelocitySize == _dimensionRowNumber)
                {
                    currentParticle.velocity2[i] = _particleInitialVelocity[i];
                }
                // The current velocity can be set from the given values.
                else if (i < _particleInitialVelocitySize)
                {
        ...
        private static int[] Shuffle(int[] index, int indexSize)
        {
            ...
            {
                int n = StaticRandom.Next(i, indexSize);
                int temp = index[n];
                index[n] = index[i];
                index[i] = temp;
            }
    ...
    class Particle
    {
        ...
        // 
        public double[] bestPosition1;

        // 
        public double[] bestPosition2;

        // 
        public double bestFitness1;

        // 
        public double bestFitness2;
    }
